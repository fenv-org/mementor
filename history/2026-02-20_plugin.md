...
```yaml
name: sessions
description: List or inspect past Claude Code sessions.
disable-model-invocation: true
user-invocable: true
argument-hint: "[list|get <session-id>] [--limit N]"
allowed-tools: "Bash"
```

Supports both `mementor sessions list` (overview with compaction_count) and
`mementor sessions get <id>` (detailed metadata: turn_count, compaction_count,
subagent_count, file_count, pr_links).

### turns (user-invocable only)

```yaml
name: turns
description: View turns from a specific session, optionally by compaction segment.
disable-model-invocation: true
user-invocable: true
argument-hint: "<session-id> [--segment N|--current] [--limit N]"
allowed-tools: "Bash"
```

Supports `--segment N` to view turns in a specific compaction segment, and
`--current` for turns after the last compaction boundary.
...
2. **Multi-angle search** (at least 3 angles):
   ...
   - Rephrase: try synonyms, broader/narrower terms, other frequently
     used languages in the project's conversations
3. **Dig into promising sessions**: sessions get → compactions → turns get
   (with --segment/--current) → scoped search
...
- [x] ~~DEPRECATED~~ Create `plugin/.claude-plugin/plugin.json`
- [x] ~~DEPRECATED~~ Write recall SKILL.md with fallback chain
- [x] ~~DEPRECATED~~ Write sessions SKILL.md
- [x] ~~DEPRECATED~~ Write turns SKILL.md
- [x] ~~DEPRECATED~~ Write compactions SKILL.md
- [x] ~~DEPRECATED~~ Write find-related SKILL.md with `${CLAUDE_SESSION_ID}`
- [x] ~~DEPRECATED~~ Write memory-researcher agent markdown
- [x] ~~DEPRECATED~~ Create `plugin/hooks/hooks.json` (Stop + PreCompact)
- [x] ~~DEPRECATED~~ Create `.claude-plugin/marketplace.json` at repo root
- [x] ~~DEPRECATED~~ Rewrite `mementor enable`:
  - [x] ~~DEPRECATED~~ Remove all mementor hooks from settings.json
  - [x] ~~DEPRECATED~~ Add extraKnownMarketplaces + enabledPlugins to settings.local.json
  - [x] ~~DEPRECATED~~ Preserve existing settings content
  - [x] ~~DEPRECATED~~ Add model download check
- [x] ~~DEPRECATED~~ Remove `HookCommand::UserPromptSubmit`, `PreToolUse`, `SubagentStart`
- [x] ~~DEPRECATED~~ Remove `hooks/prompt.rs`, `hooks/pre_tool_use.rs`, `hooks/subagent_start.rs`
- [x] ~~DEPRECATED~~ Remove `hooks/input.rs` structs for deleted hooks
- [x] ~~DEPRECATED~~ Remove `pipeline/query.rs` module
- [x] ~~DEPRECATED~~ Remove dead query functions from `queries.rs`
- [x] ~~DEPRECATED~~ Remove `commands/query.rs`
- [x] ~~DEPRECATED~~ Update CLAUDE.md documentation
- [x] ~~DEPRECATED~~ Integration tests for `mementor enable` (verify settings file changes)
